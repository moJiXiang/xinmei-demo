$(document).ready(function() {
    ...
    $(function() {
        ...
        d3.json('./data/les-mis/config.json', function(data) {
            config = data;
        });


        // loading data
        // d3.json('./data/les-mis/objects.json', function(data) {
        d3.json('./data/gongsi.json', function(data) {
            var objects = {};
            var names = [];
            ...
            for (var i = 0; i < data.length; i++) {
                var str = data[i].name;
                // var str = data[i].name + '_'+ data[i].type
                objects[str] = data[i];
                // objects[data[i].type] = data[i].type;
                objects[str].dependedOnBy = [];

            }
            console.log(objects);
            for (var i = 0; i < names.length; i++) {
                for (var j = 0; j < data[i].depends.length; j++) {
                    // console.log(data[i].depends[j]);
                    // if(objects[data[i].depends[i]]) {

                        objects[data[i].depends[j]].dependedOnBy.push(names[i]);
                    // }
                }
            }
            // console.log(objects);
